...
export default function HotelBookingPage() {
        ...
        const fetchHotelData = async () => {
            try {
                ...
                if (!selectedRoom && hotel.roomCategories.length > 0) {
                    const availableRooms = hotel.roomCategories.filter(room => room.availableRoomCount > 0);
    ...
    useEffect(() => {
        const fetchRoomAvailability = async () => {
            ...
            try {
                ...
                const response = await axios.get(
                    `${import.meta.env.VITE_BACKEND_URL}/api/hotelbooking/availability/${hotel_id}`,
                    {
                ...
                if (response.data.success) {
                    ...
                    toast.error(response.data.message || 'Failed to check room availability');
                    setRoomAvailability([]);
                }
            } catch (error) {
    ...
    const handleRoomsChange = (newRooms) => {
        ...
        if (roomCount > 0) {
            ...
            if (currentGuests > maxGuests) {
                setErrors(prev => ({
                    ...prev,
                    guests: `Maximum ${maxGuests} guests allowed for ${roomCount} room${roomCount > 1 ? 's' : ''}`
                }));
                setGuests('');
            } else {
            ...
            const availabilityData = roomAvailability.find(r => r.roomType === selectedRoom?.categoryName);
    ...
    const validateForm = () => {
        ...
        const availabilityData = roomAvailability.find(r => r.roomType === selectedRoom?.categoryName);
        if (selectedRoom && availabilityData && roomCount > availabilityData.availableRooms) {
        ...
        if (guestCount > roomCount * 4) {
            newErrors.guests = `Maximum ${roomCount * 4} guests allowed for ${roomCount} room${roomCount > 1 ? 's' : ''}`;
        }
            ...
            <div className="min-h-screen bg-gray-50 p-5 flex items-center justify-center">
                <div className="text-center bg-white p-8 rounded-lg border">
                    ...
                    <div className="text-gray-600 mb-6">
                        {!hotelData ? 'Hotel not found.' : 
                         hotelData.status !== 'approved' ? 'Hotel is not currently accepting bookings.' :
        ...
        return (
            <div className="min-h-screen bg-gray-50 p-5 flex items-center justify-center">
                <div className="max-w-md mx-auto bg-white p-8 rounded-lg border border-gray-200 text-center">
                    ...
                    <div className="mb-4">
                        <p className="text-sm text-gray-600"><strong>Hotel:</strong> {hotelData.name}</p>
                        <p className="text-sm text-gray-600"><strong>Room:</strong> {selectedRoom.categoryName}</p>
                        <p className="text-sm text-gray-600"><strong>Check-in:</strong> {new Date(checkInDate).toLocaleDateString()}</p>
                        <p className="text-sm text-gray-600"><strong>Check-out:</strong> {new Date(checkOutDate).toLocaleDateString()}</p>
                        <p className="text-sm text-gray-600"><strong>Total:</strong> LKR {totalPrice.toLocaleString()}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <div className="bg-white p-8 rounded-lg mb-6 border border-gray-200">
                            ...
                            {availabilityLoading ? (
                                <div className="text-center text-gray-600">Checking room availability...</div>
                            ) : (
                                <div className="grid grid-cols-1 gap-4">
                                    {hotelData.roomCategories
                                        .filter(room => {
                                            const availabilityData = roomAvailability.find(r => r.roomType === room.categoryName);
                                            return availabilityData?.available && availabilityData?.availableRooms > 0;
                                        })
                                        .map((room) => {
                                            const availabilityData = roomAvailability.find(r => r.roomType === room.categoryName);
                                            return (
                                                ...
                                                >
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <h3 className="font-bold text-lg">{room.categoryName}</h3>
                                                            <p className="text-sm text-gray-600 mb-2">
                                                                {availabilityData?.availableRooms || 0} room{availabilityData?.availableRooms > 1 ? 's' : ''} available
                                                            </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    >
                                        ...
                                        {selectedRoom && roomAvailability.find(r => r.roomType === selectedRoom.categoryName)?.availableRooms && (
                                            [...Array(Math.min(roomAvailability.find(r => r.roomType === selectedRoom.categoryName).availableRooms, 10))].map((_, i) => (
                                    ...
                                    {rooms && selectedRoom && (
                                        <div className="text-xs text-gray-500 mt-1">
                                            {parseInt(rooms) || 0} room{(parseInt(rooms) || 0) > 1 ? 's' : ''} = Maximum {(parseInt(rooms) || 0) * 4} guests allowed
                                        </div>
                                    )}
                                    {errors.rooms && (
                                        <span className="text-red-500 text-sm">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {rooms && [...Array((parseInt(rooms) || 0) * 4)].map((_, i) => (
                                            <option key={i + 1} value={i + 1}>
                                                {i + 1} guest{i > 0 ? 's' : ''}
                                            </option>
                                        ))}
                        ...
                        <div className="bg-white p-8 rounded-lg border border-gray-200 sticky top-5">
                            ...
                            {selectedRoom && (
                                <div className="border-b border-gray-200 pb-4 mb-4">
                                    ...
                                    <div className="text-xs text-gray-500">
                                        {roomAvailability.find(r => r.roomType === selectedRoom.categoryName)?.availableRooms || 0} room{(roomAvailability.find(r => r.roomType === selectedRoom.categoryName)?.availableRooms || 0) > 1 ? 's' : ''} available
                                    </div>
                                </div>
                            )}
                            {numberOfNights > 0 && selectedRoom && (
                                <div className="border-b border-gray-200 pb-4 mb-4">
                                    ...
                                    <div className="flex justify-between mb-2">
                                        <span>Rooms:</span>
                                        <span className="font-semibold">{parseInt(rooms) || 0}</span>
                                    </div>
                                    <div className="flex justify-between mb-2">
                                        <span>Guests:</span>
                                        <span className="font-semibold">{parseInt(guests) || 0}</span>
                                    </div>
                                </div>
                            )}
                            {numberOfNights > 0 && selectedRoom && (
                                <div className="border-b border-gray-200 pb-4 mb-4 text-sm">
                                    <div className="flex justify-between mb-1 text-gray-600">
                                        <span>{selectedRoom.categoryName} × {parseInt(rooms) || 0} room{(parseInt(rooms) || 0) > 1 ? 's' : ''}</span>
                                        <span>LKR {((selectedRoom.pricePerNight * (parseInt(rooms) || 0))).toLocaleString()}</span>
                                    </div>
                            ...
                            {selectedRoom && selectedRoom.facilities && selectedRoom.facilities.length > 0 && (
                                <div className="border-b border-gray-200 pb-4 mb-4">
                                    <div className="text-sm font-semibold mb-2">Room Includes:</div>
                                    <div className="text-xs text-gray-600">
                                        {selectedRoom.facilities.slice(0, 4).map((facility, index) => (
                                            <div key={index} className="mb-1">• {facility}</div>
                                        ))}
                                        {selectedRoom.facilities.length > 4 && (
                                            <div className="text-blue-600">+{selectedRoom.facilities.length - 4} more facilities</div>
                                        )}
                                    </div>
                                </div>
                            )}
                            <div className="border-b-2 border-green-600 pb-4 mb-6">
                                ...
                                {numberOfNights > 0 && (
                                    <div className="text-xs text-gray-600 text-right mt-1">
                                        ({numberOfNights} night{numberOfNights > 1 ? 's' : ''} × {parseInt(rooms) || 0} room{(parseInt(rooms) || 0) > 1 ? 's' : ''})
                                    </div>
                                )}
                            </div>
                            ...
                            <button
                                ...
                                disabled={isLoading || totalPrice === 0 || !selectedRoom || availabilityLoading}
                                className={`w-full py-4 rounded-lg text-lg font-bold transition-colors ${
                                    isLoading || totalPrice === 0 || !selectedRoom || availabilityLoading
