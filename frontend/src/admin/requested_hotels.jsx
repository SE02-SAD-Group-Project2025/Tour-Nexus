...
export default function RequestedHotelDetails() {
    ...
    const approveHotel = async () => {
        if (confirm(`Approve ${hotel.name}?`)) {
            try {
                const token = localStorage.getItem('token');
                
                const response = await axios.put(
                    `${import.meta.env.VITE_BACKEND_URL}/api/hotel/approve_hotel/${hotel.hotel_id}`,
                    { status: 'approved' },
                    {
                        headers: {
                ...
                if (error.response?.status === 403) {
                    toast.error("You don't have permission to approve hotels. Admin access required.");
                } else {
    ...
    const rejectHotel = async () => {
        const reason = prompt(`Reason for rejecting ${hotel.name}:`);
        if (reason) {
            try {
                const token = localStorage.getItem('token');
                
                const response = await axios.put(
                    `${import.meta.env.VITE_BACKEND_URL}/api/hotel/reject_hotel/${hotel.hotel_id}`,
                ...
                toast.success(`${hotel.name} rejected. Reason: ${reason}`);
                setHotel(prev => ({ ...prev, status: 'rejected' }));
            } catch (error) {
                console.error("Error rejecting hotel:", error);
                if (error.response?.status === 403) {
                    toast.error("You don't have permission to reject hotels. Admin access required.");
                } else {
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <div>
                                <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '500' }}>HOTEL ID</div>
                                <div style={{ fontWeight: '600', color: '#1e293b' }}>{hotel.hotel_id}</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <span style={{ fontSize: '20px' }}>🏙️</span>
                            <div>
                                <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '500' }}>CITY</div>
                                <div style={{ fontWeight: '600', color: '#1e293b' }}>{hotel.city}</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <span style={{ fontSize: '20px' }}>📞</span>
                            <div>
                                <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '500' }}>HOTLINE NUMBER</div>
                                <div style={{ fontWeight: '600', color: '#1e293b' }}>{hotel.contact_number}</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <span style={{ fontSize: '20px' }}>📅</span>
                                ...
                                {currentRoom.images && currentRoom.images.length > 0 && (
                                    <div>
                                        ...
                                        <div style={{
                                            ...
                                            📷 {currentRoom.images.length} Room Image{currentRoom.images.length !== 1 ? 's' : ''} Available
                                        </div>
                                    </div>
                                )}
                            </div>
